 ...
 *
 *
 *This class is a fork of WC_Geolocation https://github.com/woothemes/woocommerce/blob/56520cd8bc78f0f1db550b660523ec174671434f/includes/class-wc-geolocation.php 
 *
 * Geolocation class Wolive
...
class Wolive_Geolocation {
	...
	public static function get_external_ip_address() {
		...
		if ( false === $external_ip_address ) {
			...
			$ip_lookup_services      = apply_filters( 'wolive_geolocation_ip_lookup_apis', self::$ip_lookup_apis );
			...
			foreach ( $ip_lookup_services_keys as $service_name ) {
				...
				if ( ! is_wp_error( $response ) && $response['body'] ) {
					$external_ip_address = apply_filters( 'wolive_geolocation_ip_lookup_api_response',  $response['body'], $service_name );
					break;
				}
			}
	...
	public static function geolocate_ip( $ip_address = '', $fallback = true, $api_fallback = true ) {
		...
		$country_code = apply_filters( 'wolive_geolocate_ip', false, $ip_address, $fallback, $api_fallback );
	...
	public static function get_local_database_path( $version = 'v4' ) {
		...
		return apply_filters( 'wolive_geolocation_local_database_path', $upload_dir['basedir'] . '/GeoIP' . $version . '.dat', $version );
	}
	/**
	...
	private static function geolocate_via_db( $ip_address ) {
		if ( ! class_exists( 'Wolive_Geo_IP' ) ) {
			require_once( WOLIVE__PLUGIN_DIR . 'includes/class.wolive.geoip.php');
		}
		$gi = new Wolive_Geo_IP();
		if ( self::is_IPv6( $ip_address ) ) {
			$database = self::get_local_database_path( 'v6' );
	...
	private static function geolocate_via_api( $ip_address ) {
		...
		if ( false === $country_code ) {
			$geoip_services      = apply_filters( 'wolive_geolocation_geoip_apis', self::$geoip_apis );
			...
			foreach ( $geoip_services_keys as $service_name ) {
				...
				if ( ! is_wp_error( $response ) && $response['body'] ) {
					switch ( $service_name ) {
						...
						break;
						default :
							$country_code = apply_filters( 'wolive_geolocation_geoip_response_' . $service_name, '', $response['body'] );
						break;
					}
