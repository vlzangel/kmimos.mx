<?php foreach ( $addon['options'] as $key => $option ) :
	...
	<p class="form-row form-row-wide addon-wrap-<?php echo sanitize_title( $addon['field-name'] ); ?>">
		...
		<?php endif; ?>
		<input type="text" class="input-text addon addon-custom" data-price="<?php echo get_product_addon_price_for_display( $option['price'] ); ?>" id="<?php echo $option_key; ?>" name="<?php echo $addon_key ?>[<?php echo $option_key; ?>]" value="<?php echo esc_attr( $current_value ); ?>" <?php if ( ! empty( $option['max'] ) ) echo 'maxlength="' . $option['max'] .'"'; ?> />
	</p>
