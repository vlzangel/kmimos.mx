...
/*						$item_count = $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM $wpdb->posts where post_author = %d and post_type = %s and post_status IN (%s,%s,%s)",$user_id,"pets","publish","pendingpayment","pendingapproval")  );*/
                        ...
                        */
                        if($_GET['ua']=='invoices'){
                            $pfmenu_output .= '<li class="selected_option"><a href="#" onclick="return false;"><i class="pfadmicon-glyph-33"></i> '. $setup29_dashboard_contents_vendor_purchases_menuname.'</a></li>';
                        }
                        else {
                            $class = ($_GET['ua']=='invoices')? ' class="selected_option"':'';
                            $pfmenu_output .= '<li'.$class.'><a href="'.$setup4_membersettings_dashboard_link.$pfmenu_perout.'ua=invoices"><i class="pfadmicon-glyph-33"></i> '. $setup29_dashboard_contents_vendor_purchases_menuname.'</a></li>';
                        }
...
*/
					...
					**/
					...
					switch ($ua_action) {
                        ...
                        case 'myshop':
                            /**
                            *Start: My Shop Page Content
                            **/
                            echo '<h1>Mi Perfil como Cuidador</h1><hr><br>';
									...
									if (esc_attr($_POST['action']) == 'pfupdate_my_shop') {
										...
										if($user_id == 0){
                                            ...
                                            */
                                            ...
                                            $petsitter_name = $meta_user['first_name'][0].' '.$meta_user['last_name'][0];
                                            // Busca el cuidador asignado al usuario
                                            $args = array(
                                                'posts_per_page' => 1,
                                            ...
                                            $petsitter_metas = get_post_meta($petsitter_id);
                                            $petsitter_code = $petsitter_metas['code_petsitter'][0];
                                            $rating_petsitter = $petsitter_metas['rating_petsitter'][0];

                                            if( $rating_petsitter == "" ){
                                            	update_post_meta($petsitter_id, "rating_petsitter", 0);
                                            }
					                        ...
					                        if( $_POST['latitude_petsitter'] == '' ){ $_POST['latitude_petsitter'] = "4.570868"; }
					                        if( $_POST['longitude_petsitter'] == '' ){ $_POST['latitude_petsitter'] = "-74.29733299999999"; }

                                            $metas = array(
                                                'address_petsitter' => wp_strip_all_tags( $_POST['shop_address'] ),
                                                'sector_petsitter' => wp_strip_all_tags( $_POST['shop_sector'] ),
                                                'zip_petsitter' => wp_strip_all_tags( $_POST['shop_zip'] ),
                                                'housing_petsitter' => wp_strip_all_tags( $_POST['shop_type'] ),
                                                'greens_petsitter' => (($_POST['shop_green']==1)? 1:0),
                                                'yard_petsitter' => (($_POST['shop_yard']==1)? 1:0),
                                                'latitude_petsitter' => wp_strip_all_tags( $_POST['latitude_petsitter'] ),
                                                'longitude_petsitter' => wp_strip_all_tags( $_POST['longitude_petsitter'] ),
                                                'pets_puppies' => (($_POST['puppies_accepted']==1)? 1:0),
                                                'pets_adults' => (($_POST['adults_accepted']==1)? 1:0),
                                                'pets_seniors' => (($_POST['seniors_accepted']==1)? 1:0),
                                                'pets_males' => (($_POST['males_accepted']==1)? 1:0),
                                                'pets_females' => (($_POST['females_accepted']==1)? 1:0),
                                                'pets_sterilized' => (($_POST['only_sterilized']==1)? 1:0),
                                                'pets_unsociable' => (($_POST['not_sociables']==1)? 1:0),
                                                'aggressive_with_humans' => (($_POST['aggresive_humans']==1)? 1:0),
                                                'aggressive_with_pets' => (($_POST['aggresive_pets']==1)? 1:0),
                                                'pets_petsitter' => ($_POST['pets_petsitter']),
                                                'mascotas_hospedaje' => ($_POST['mascotas_hospedaje']),
                                                'sizes_pets' => (serialize($tamanos)),
                                                'home_pets' => (serialize($tengo))
                                            );
                                            
                                            $args = array(
                                                'ID'            => $petsitter_id,
                                                'post_content'  => wp_strip_all_tags( $_POST['shop_description'] ),
                                                'post_status'   => 'publish',
                                                'post_author'   => $user_id,
                                                'post_type'     => 'petsitters',
                                                'meta_input'    => $metas
                                            ...
                                            if ( isset($_FILES['petsitter_photo'])) {   
                                                if ( $_FILES['petsitter_photo']['size'] > 0) { 
                                                    $photo_name = str_pad($petsitter_code, 4, '0', STR_PAD_LEFT).'_000p';
                                                    $file = array(
                                                    ...
                                                    if(!in_array($_FILES['petsitter_photo']['type'], $allowed_file_types)) { // wrong file type
                                                      $errorval .= esc_html__("Please upload a JPG, GIF, or PNG file.<br>",'pointfindert2d');
                                                    	...
                                                    	$id_img = media_handle_sideload( $_FILES['petsitter_photo'], $petsitter_id );
	                                                    ...
	                                                    update_post_meta($petsitter_id, '_thumbnail_id', $id_img);
                        ...
                        case 'myservices':
                            ...
                            **/
                            echo '<h1>Mis Servicios</h1><hr><br>';
                            $output = new PF_Frontend_Fields(
                                array(
                                    'formtype' => 'myservices',
                                    'current_user' => $user_id,
                                                ...
                                                if(!in_array($_FILES['petsitter_photo']['type'], $allowed_file_types)) { // wrong file type
                                                  $errorval .= esc_html__("Please upload a JPG, GIF, or PNG file.<br>",'pointfindert2d');
                                                    ...
                                                    foreach ($_FILES as $file => $array) {
                                                        ...
                                                        //  600px de alto
                                                        //
                                                        $newuploadphoto = kmimos_upload_and_process_image(800, 600, $photo_name, $array);
                                                        $my_post = array(
                                                            'ID'           => $newuploadphoto,
                                                            'post_title'   => $photo_name,
                                                            'post_content' => '',
                       ...
                       case 'mysales':
                        ...
                        case 'mypets':
                            ...
                            **/
                            ...
                            echo '<p align="justify">Si piensas contratar un servicio a través de Kmimos, es importante que identifiques tus mascotas ya que en todos nuestros servicios tus mascotas estarán cubiertas por una póliza de seguros.</p>';
                            echo '<p align="justify">Si piensas formar parte de la familia de Cuidadores de Kmimos es importante también que tus mascotas estén identificadas. Muchas personas prefieren contratar a cuidadores que tengan sus propias mascotas, otros buscan cuidadores que tengan mascotas de determinadas razas y tamaños.</p><br><hr>';
