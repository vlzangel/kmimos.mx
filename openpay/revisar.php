<?php
	...
	if( count($ordenes_pasadas) > 0){

		$ordenes = array();
		foreach ($ordenes_pasadas as $f) {
			$ordenes[] = $f->orden;
		}
			...
			if( in_array($value->order_id, $ordenes) ){
				...
				switch ($value->status) {
					...
					case 'cancelled':
						...
						$id_reserva = $wpdb->get_var("SELECT * FROM wp_posts WHERE post_parent = {$id_orden} AND post_type LIKE 'wc_booking'");
					...
					case 'completed':
						...
						$booking = new WC_Booking( $id_reserva );
   						$order   = new WC_Order($id_orden);
							...
							wp_mail( $email_admin, 		'Nueva Reserva - '.$tipo_servicio.' por: '.$nombre.' '.$apellido, $mensaje_admin);
							wp_mail( $cuidador->email, 	'Nueva Reserva - '.$tipo_servicio.' por: '.$nombre.' '.$apellido, $mensaje_cuidador);

							$administradores = array(
								"e.celli@kmimos.la",
								"r.gonzalez@kmimos.la"
							);

							$titulo = 'Copia Administradores: Nueva Reserva - '.$tipo_servicio.' por: '.$nombre.' '.$apellido;

							wp_mail( $administradores[0], $titulo, $mensaje_cuidador);
							wp_mail( $administradores[1], $titulo, $mensaje_cuidador);
							wp_mail( $administradores[2], $titulo, $mensaje_cuidador);
